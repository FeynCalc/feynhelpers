...
"FIREAddPropagators is an option for FIREBurn. Normally, for loop integrals
that do not have enough propagators to form a complete basis, FIREBurn will
automatically include missing propagators and put them to unity after the
reduction is complete. In some cases it may be desirable to choose the missing
propagators manually. This can be done by specifying the  propagators via
AddPropagators->{prop1,prop2,...}.";

FIRESilentMode::usage=
"FIRESilentMode is an option for FIREBurn. When set to True, all the
Print-output of FIRE running on the parallel kernel will be suppressed. This
does not affect messages, i.e. warnings or error messages will be still
visible.";

FIREStartFile::usage=
"FIREStartFile is an option for FIREBurn. It specifies, where the start file
for FIRE (the one generated by SaveStart[]) will be saved. The default
location is the Database folder inside $FeynCalcDirectory.";
...
FIRERun::usage=
"FIRERun is an option for FIREBurn. When set to False, the interface will only
create configuration files specified in FIREConfigFiles but will not run FIRE
on these files.";

FIREUsingFermat::usage=
"FIREUsingFermat is an option for FIREBurn. When set to True, FIRE will use
FERMAT by R. H. Lewis to speed up the reduction. The default value is False.

Before you activate FERMAT, please make sure that you have read and understood
its license agreement.";

FIREBurn::usage=
"FIREBurn[expr, {q1, q2, ...}, {p1, p2, ...}] reduces loop integrals with loop
momenta q1, q2, ... and external momenta p1, p2, ... with integration-by-parts
(IBP) relations.

FIREBurn expects that the input does not contain any loop integrals with
linearly dependent propagators. Therefore, prior to starting the reduction,
use ApartFF.

The evaluation is done on a parallel kernel using A.V. Smirnov's and V.A.
Smirnov's FIRE.";

FIREConfigFiles::usage=
"FIREConfigFiles is an option for FIREBurn. It specifies, where the three files
that contain all the FIRE configuration are saved.

The first file contains the lists of loop momenta, external momenta and
propagators. Normally it ends with the FIRE command SaveStart.
...
abbreviations. The default location of these files is the Database folder
inside $FeynCalcDirectory.";
