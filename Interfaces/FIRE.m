...
Options[FIREBurn] = {
	...
	FIREConfigFiles -> {ToFileName[{$FeynCalcDirectory, "Database"}, "FIREp1.m"],
						ToFileName[{$FeynCalcDirectory, "Database"}, "FIREp2s.m"]},
	FIRESilentMode -> True,
	FCVerbose -> False
};
...
batchFIRE[qs_List,ext_List,props_List,opts:OptionsPattern[FIREBurn]]:=
	(prepareFIRE[qs,ext,props,opts]; RunFIRE[OptionValue[FIREConfigFiles],
...
prepareFIRE[qs_List,ext_List,props_List,OptionsPattern[FIREBurn]]:=
	Block[{	internal,external,prs,propagators,replacements,
			...
			fireConfigPath1,fireConfigPath2},
		...
		WriteString[fireConfig1, "Get[\"FIRE5`FIRE5`\"];\n"];
		...
		WriteString[fireConfig2, "Get[\"FIRE5`FIRE5`\"];\n"];
...
RunFIRE[{fireConfigPath1_,fireConfigPath2_},OptionsPattern[FIREBurn]]:=
	Block[{tmp,propagators,qs,kernel, outFIRE, gList, pList,g,repList,solsList,res,null1,null2},
